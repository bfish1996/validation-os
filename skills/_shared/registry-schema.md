...
| Stage | select | The **kind of external-actor response** the belief tests — the discovery taxonomy (`docs/stage-policy.md`). Four values, stored by name: `Discovery` (problem-solution fit — will they engage / care / disclose?), `Validation` (product-market fit — will they pay / sign / stay?), `Scale` (growth — can we acquire efficiently, does CAC<LTV hold at volume?), `Maturity` (defense — will incumbents respond, will regulators accept?). **Required.** The membership test is the subject-verb rule: every assumption's subject must be an external actor (user, buyer, competitor, regulator, partner, distributor, investor) or the market, never "we". There is no stage for "we can build X" — build risk isn't a stage of discovery, it's a delivery phase, and a claim that doesn't fit any stage falls out of the register at write time. The stored value is the **name** (the ordinal 1–4 is for sorting only, never written). **Orthogonal to `Lens`** — `Lens` is *who* the belief is about (an audience), `Stage` is *what kind of response* is being tested (engage / pay / scale / defend). A Lens can appear at multiple stages: Commercial spans all four; Consumer spans 1–3 and is zero at Stage 4 (consumers don't drive defense bets). No Lens maps 1:1 to a stage — both fields earn their keep, and collapsing them loses signal. The integrity rule `stage-actor-consistency` (warn) flags a claim whose subject is inconsistent with its stage's expected actor — advisory, surfaced by `/assumptions audit`. **Stage sets the Risk threshold for action** (the question-type-aware evidence ladder): `RISK_THRESHOLD_BY_STAGE` (Discovery 30, Validation 15, Scale 10, Maturity 5) — the stopping rule, pragmatic encroachment + Bezos two-way/one-way doors. The threshold does NOT flip a status; it is a prioritisation rule for attention. |
| Question Type | select | The **kind of claim** the belief raises (the question-type-aware evidence ladder, `docs/question-types.md`). Seven values: `Existence` (does the pain/mechanism exist?), `Prevalence` (what's the rate?), `CausalEffect` (does X cause Y?), `WillingnessToPay` (will they pay / commit?), `ValueUtility` (do they derive value / keep using it?), `Regulatory` (will the regulator approve?), `Feasibility` (can they do it / can we build it?). **Required.** Set by the **falsification-test rule**: the question type is determined by what would prove the assumption WRONG, not by what evidence is cheap — "users will pay $50/mo" is falsified by offering it and watching them not pay → WillingnessToPay, regardless of how many interview quotes exist. The grill infers the type from the falsification bar and enforces the **gaming guard**: the inferred type must match the stated type, or the Draft → Live promotion is rejected. **Orthogonal to `Stage`** (kind of response) and `Lens` (who the actor is) — see `docs/stage-policy.md § The Question Type × Stage orthogonality`. The question type determines which rungs are probative, what the ceiling is, and which rungs are **non-evidence** (anchor 0) — via `RUNG_ANCHOR[questionType][rung][band]` (`docs/evidence-ladder.md`). |
...
| Completeness % | derived | **Never hand-write.** = filled slots / all slots × 100, over six structural slots: Description, Lens, Impact, Scoring justification, dependencies traced (≥1 `Depends on`/`Enables` link), **Question Type** (the question-type-aware evidence ladder). The Draft⇔open-work readiness meter — 100 = Live-ready, below 100 = open work. Replaces the retired Gaps/presence-field machinery (`the evidence-remodel slice`; `assumption-guardrails.md §3`). The `Question Type` slot is the Live gate added by the question-type-aware evidence ladder — an assumption without a Question Type cannot go Live. |
...
| Rung | select | **Row-level — one rung per artifact, per reading** (0.10: rung is a property of the artifact, not the belief). The ladder rung this whole artifact sits on, judged from what it shows. The 6-rung vocabulary is **fixed across all question types** (the question-type-aware evidence ladder): 🧪 Testing — `Talk` (the collapsed floor — Opinion + Pitch-deck + Anecdotal merged) · `Desk research` (regulation, published data, competitor facts) · `Signed up` (consumer lens's first do-rung — fake-door signup) · `Observed usage` (genuine usage sessions, sustained retention, A/B tests); 🎯 Market — `Signed intent` (LOI, deposit, costly commitment before build) · `Paying users` (real money, A/B on live traffic, signed contract). The **anchor** (ceiling `s`) is per **(question type × rung × band)** — see `RUNG_ANCHOR[questionType][rung][band]` in `docs/evidence-ladder.md`. A rung that is **non-evidence** for the linked assumption's question type carries anchor 0 across all bands (contributes `s=0`, flagged at the UI/skill layer — not a write blocker). An artifact that spans two rungs is **split into separate readings, one per rung** (`experiment-guardrails.md §0`). |
