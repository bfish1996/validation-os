...
| Completeness % | derived | **Never hand-write.** = filled slots / all slots × 100, over six structural slots: Description, Lens, Impact, Scoring justification, dependencies traced (≥1 `Depends on`/`Enables` link), **Question Type** (DEV-5890). The Draft⇔open-work readiness meter — 100 = Live-ready, below 100 = open work. Replaces the retired Gaps/presence-field machinery (`OPS-1305`; `assumption-guardrails.md §3`). The `Question Type` slot is the Live gate added by the question-type-aware evidence ladder — an assumption without a Question Type cannot go Live. |
