...
| Stage | select | **Retained only for migration reading of legacy records.** The discovery taxonomy (`Discovery` / `Validation` / `Scale` / `Maturity`) was retired by the confidence-scoring simplification. Its IDEO-triangle meaning is absorbed by `Risk Group`; its reversibility meaning is dropped. Null on every post-simplification belief. |
| Question Type | select | **Retained only for migration reading of legacy records.** The 7 academic question types (`Existence` / `Prevalence` / `CausalEffect` / `WillingnessToPay` / `ValueUtility` / `Regulatory` / `Feasibility`) were retired and replaced by `Assumption Type`. Null on every post-simplification belief. |
| Assumption Type | select | The **evidence key** — what kind of claim the belief is, set by what would prove it false (the gaming guard). Eleven values: `ProblemExists` · `ProblemWidespread` · `WantOurSolution` · `ItWorks` · `CanCompleteTask` · `CanBuildIt` · `LegalCompliant` · `TheyllPay` · `TheyKeepUsingIt` · `ReachProfitably` · `EconomicsWork`. **Inferred on write** from the falsification bar (`wrongIf`) of any experiment naming the belief, falling back to the description — no dropdown, no hand-picking. A living inference: re-runs on every touching write, sharpening once a bar exists. A brand-new un-grilled belief lands on the permissive `ProblemExists` default. Each type maps to exactly one `Risk Group` and determines which rungs are probative, the ceiling, and which rungs are non-evidence — via `RUNG_ANCHOR[assumptionType][rung][band]` (`docs/evidence-ladder.md`). |
| Risk Group | select (derived) | The foreground headline axis: `Desirability` · `Usability` · `Feasibility` · `Viability`. Derived from the Assumption Type via `TYPE_TO_GROUP`, not separately hand-set. |
| Cost to test | select (derived) | `cheap` · `moderate` · `expensive` — derived from the Assumption Type's ceiling-rung nature. Overridable per assumption. |
| Graduation | select (derived) | `Untested` · `Signal` · `Graduated` — based on Confidence vs the graduation bar (a function of Derived Impact). The progression: no evidence → Untested; evidence but below the bar → Signal; Confidence ≥ bar → Graduated. |
...
| Completeness % | derived | **Never hand-write.** = filled slots / all slots × 100, over six structural slots: Description, Lens, Impact, Scoring justification, dependencies traced (≥1 `Depends on`/`Enables` link), **Assumption Type**. The Draft⇔open-work readiness meter — 100 = Live-ready, below 100 = open work. The `Assumption Type` slot is the Live gate — an assumption without an inferred type cannot go Live. |
...
| Rung | select | **Row-level — one rung per artifact, per reading.** The 11-rung vocabulary is **fixed across all assumption types**: `Talk` (stated opinion — interviews) · `Survey` (stated opinion at scale) · `Desk & data` (secondary research / public data) · `Fake-door` (pretended offering, observed signups) · `Prototype use` (observed usage of a prototype) · `Retention` (sustained usage over time) · `Commitment` (signed intent — LOI, design partner agreement) · `Payment` (real money paid) · `Build proof` (operational proof the system can be built) · `Outcome test` (causal/efficacy test — A/B, pre/post) · `Cost data` (unit economics data). The **anchor** (ceiling `s`) is per **(assumption type × rung × band)** — see `RUNG_ANCHOR[assumptionType][rung][band]` in `docs/evidence-ladder.md`. A rung that is **non-evidence** for the linked assumption's type carries anchor 0 across all bands (contributes `s=0`, flagged at the UI/skill layer — not a write blocker). An artifact that spans two rungs is **split into separate readings, one per rung** (`experiment-guardrails.md §0`). |
