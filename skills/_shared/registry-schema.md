...
- **Experiments** — the unified evidence **plan** (Testing and Market-grade
  both live here — the Goal record is gone, `the evidence-remodel slice`). One row per designed
...
| Stage | *(deprecated)* | **Retired** (the confidence-scoring simplification, OPS-1406/OPS-1418). Its IDEO-triangle meaning (which kind of external-actor response the belief tests) is absorbed by `Risk Group`, derived from `Assumption Type`; its reversibility/stage-threshold meaning is dropped outright — the impact-scaled graduation bar replaces it (below). The field is kept only so a legacy row's stored `Stage` value can still be read during migration; nothing writes it going forward. |
| Assumption Type | derived | **Never hand-entered — inferred on every write.** The kind of claim the belief raises: 11 values (`ProblemExists`, `ProblemWidespread`, `WantOurSolution`, `ItWorks`, `CanCompleteTask`, `CanBuildIt`, `LegalCompliant`, `TheyllPay`, `TheyKeepUsingIt`, `ReachProfitably`, `EconomicsWork` — `docs/evidence-ladder.md`). Set by the **falsification-test rule**: the type is determined by what would prove the assumption WRONG, not by what evidence is cheap to gather — "users will pay $50/mo" is falsified by offering it and watching them not pay → `TheyllPay`, regardless of how many interview quotes exist. `inferAssumptionType(description, wrongIfBar)` reads the bar-line `wrongIf` text of a bar naming the assumption if one exists, else falls back to the assumption's `Description`, and re-runs on every touching write (`packages/core/src/derivation/assumption-type.ts`, wired into `recompute.ts`) — so the type **sharpens once grilled**: a vague bar infers the most permissive type (`ProblemExists`, flagged for review); a sharpened bar infers the right one automatically, no separate state to hand-maintain. This is the **gaming guard**: it stops a team reframing "will users pay?" as "do users express willingness to pay?" to duck a market test. Determines which rungs are probative, the ceiling, and which rungs are **non-evidence** (anchor 0) — via `RUNG_ANCHOR[assumptionType][rung][band]` (`docs/evidence-ladder.md`) — and derives `Risk Group` (below). |
| Risk Group | derived | **Never hand-write.** One of `Desirability` / `Usability` / `Feasibility` / `Viability`, via `TYPE_TO_GROUP[assumptionType]` — a fixed 11→4 map, never separately set. The foreground headline axis; currently unsurfaced in the dashboard but live infrastructure. |
| Cost to test | derived | **Never hand-write.** `cheap` / `moderate` / `expensive`, derived from the assumption type's ceiling-rung nature (talk/desk → cheap; prototype/usability → moderate; sustained behaviour / money / operational → expensive). Overridable per assumption — context can bend it. |
| Graduation | derived | **Never hand-write.** `Untested` / `Signal` / `Graduated` — the progression against the impact-scaled **graduation bar** `min(40 + 0.5 × Derived Impact, 90)` (`packages/core/src/derivation/graduation.ts`). Replaces the retired Stage-keyed Risk threshold as the stopping rule: bigger bets need more proof before graduating. Recomputed on every write — a disconfirming reading can move it backwards; this never flips `Status` (`docs/validated.md`). |
...
| Derived Impact | derived | **Never hand-write.** = seed + (100 − seed) × S/(S + 100), where S sums the dependents' pull (dependent assumptions' Derived Impact + 100 per standing decision `Based on` node; an experiment never contributes). **Recomputed on every touching write** alongside Confidence and Risk — no deliberate staleness (`the derive-on-write invariant`); the batch pass is only a backstop for writes that bypass the dashboard. `assumption-guardrails.md §3`. |
...
| Confidence | derived | **Never hand-type.** Signed −100…100, 0 = no evidence: strength-weighted average of the concluded **Readings' `beliefs[]` entries scored against this belief** — every Confidence input, whatever its origin — shrunk toward 0 by a **per-rung** neutral prior W0 (`Desk & data` = 2, every other rung = 6.5 — `W0_BY_RUNG`, `docs/evidence-ladder.md`), deduped per (belief, source). ≤ −50 = the kill zone (human review prompt). Full rule: `experiment-guardrails.md §2`. |
| Completeness % | derived | **Never hand-write.** = filled slots / all slots × 100, over six structural slots: Description, Lens, Impact, Scoring justification, dependencies traced (≥1 `Depends on`/`Enables` link), **Assumption Type** (the evidence ladder, `docs/evidence-ladder.md`). The Draft⇔open-work readiness meter — 100 = Live-ready, below 100 = open work. Replaces the retired Gaps/presence-field machinery (`the evidence-remodel slice`; `assumption-guardrails.md §3`). The `Assumption Type` slot is present once the falsification bar/description infers a type at all (it always does — `ProblemExists` is the fallback); an assumption without a sharpened bar still counts this slot filled but is flagged for review (the gaming guard), never blocked from Completeness. |
...
| Owner | dashboard user | Who voiced / champions the belief and is accountable for testing it. A reference to the auth-sourced dashboard-users list (`validation-os.config.yaml`), not free text and not a register (`the evidence-remodel slice`). |
| Scoring justification | text | Why the seed `Impact` was scored as it was, incl. dated moot lines when a decision `Resolves` the belief (`decision-guardrails.md §8`). The one hand-typed rationale left on an assumption — narrowed to the Impact-seed reason only: the why-trace lives in `Depends on`/`Enables`, falsifiability is enforced by the grill, and the concrete threshold lives on the experiment bar (`the evidence-remodel slice`). |
...
Assumptions carry **no body** (`the evidence-remodel slice`) — `Scoring justification` is the
...
**Derived facets (display-only; never stored — `the evidence-remodel slice`):** a reading
...
and Market-grade both live here; the Goal record is gone, `the evidence-remodel slice`). It
...
| Instrument | link | The reusable artifact the run is built on — an interview, a dataset, an analytics cohort, or a payment event (broadened with the unification, `the evidence-remodel slice`, to cover the instruments a Goal used to carry). Readings born from this run inherit it as their `Source`. |
...
| Deadline | date (optional) | When a committed plan's bars are judged. Fixed at commit. Folded in from the retired Goal (`the evidence-remodel slice`). |
...
atomic unit Confidence reads (`the per-belief reading model`). A Reading is one artifact row
...
| Rung | select | **Row-level — one rung per artifact, per reading** (0.10: rung is a property of the artifact, not the belief). The ladder rung this whole artifact sits on, judged from what it shows. The 11-rung vocabulary is **fixed across all assumption types** (`docs/evidence-ladder.md`): `Talk` (stated opinion, interviews) · `Survey` (stated opinion at scale) · `Desk & data` (secondary research / public data) · `Fake-door` (a pretended offering, observed signups) · `Prototype use` (observed usage of a prototype) · `Retention` (sustained usage over time) · `Commitment` (signed intent — LOI, design-partner agreement) · `Payment` (real money paid) · `Build proof` (operational proof the system can be built) · `Outcome test` (a causal/efficacy test, A/B or pre/post) · `Cost data` (unit-economics data). The **anchor** (ceiling `s`) is per **(assumption type × rung × band)** — see `RUNG_ANCHOR[assumptionType][rung][band]` in `docs/evidence-ladder.md`. A rung that is **non-evidence** for the linked assumption's type carries anchor 0 across all bands (contributes `s=0`, flagged at the UI/skill layer — not a write blocker). An artifact that spans two rungs is **split into separate readings, one per rung** (`experiment-guardrails.md §0`). |
| Magnitude band | select {Low, Typical, High} | **Row-level.** The magnitude this artifact landed at on its rung, from the absolute outcome (never %-of-target). Applies to **every** rung — the anchor lookup is the same 3D shape (assumption type × rung × band) whether the rung is Talk or Payment; defaults to `Typical` when absent. Feeds each entry's `Strength`. |
| Date | date | When it was observed. |
| Owner | dashboard user (optional) | Who logged it. |
| Body | text | The reading's body, on the **canonical two-heading template**: `## Quote` (the verbatim text of what the source said or did) + `## Source` (who / when / link). Analysis and reasoning do **not** go here — they live in each `beliefs[]` entry's `Grading justification`. Reintroduced as a deliberate reversal of the the evidence-remodel slice "readings carry no body" slice (backfilled from Notion, shown in the dashboard). |

A Reading has **one origin type**: an `Experiment`, or none (a bare found
Reading) — the Goal origin is gone (`the evidence-remodel slice`).
...
| Strength | derived | **Never hand-write.** The signed reading value `s` for this belief: `RUNG_ANCHOR[assumptionType][rung][band]` (the **row-level** `Rung` × **row-level** `Magnitude band`, looked up against the linked assumption's `Assumption Type`) × sign(this entry's `Result`); 0 on `Inconclusive`. The assumption's Confidence reads this. (Rung/magnitude are per-artifact; only `Result` — and therefore the sign — is per belief.) |
...
reversal** of the the evidence-remodel slice "readings carry no body" slice (brought back from
...
| Statement | text | The one-line statement of what was decided. Promoted from the `## Decision` body (`the evidence-remodel slice`). |
...
| Owner | dashboard user | Who owns the decision. A reference to the dashboard-users list, not free text (`the evidence-remodel slice`). |
...
| Unanimity justification | text | Why the score was scored as it was. Promoted from `## Rationale` (`the evidence-remodel slice`). |
...
properties, no body (`the evidence-remodel slice`) — the terminology check parses structure
directly.
...
| Definition | text | The definition, one sentence per applicable audience if it diverges. Promoted from the `## Definition` body (`the evidence-remodel slice`). |
...
4. **Readings.** Split the old `Source` value into `Source` (keep only the
   ...
   body — this reverses the the evidence-remodel slice cut for readings); cut `## Notes`. Drop the
