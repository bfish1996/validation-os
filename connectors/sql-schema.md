...
registers:
  assumptions:
    ...
    properties:
      ...
      - {canonical: Assumption Type, backend: assumption_type, type: TEXT, derived: true, formula: "inferAssumptionType(description, wrongIf bar text) — DERIVED, never hand-entered; recomputed on every touching write (packages/core/src/derivation/assumption-type.ts)", options_source: registry-schema}
...
| Assumption Type | `assumption_type` | TEXT (11 values — see `docs/evidence-ladder.md`) | **yes** — inferred on every write, never hand-entered |
